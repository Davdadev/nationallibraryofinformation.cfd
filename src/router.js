...
const routes = [
  ...
  { re: /^\/collection\/([^/]+)\/?$/, view: CollectionDetailView, nav: "discover", crumbs: (slug) => [{ label: "Home", path: "#/" }, { label: "Discover", path: "#/discover" }, { label: collections.find(c => c.slug === slug)?.title || "Collection", path: `#/collection/${slug}` }] },
  { re: /^\/article\/([^/]+)\/?$/, view: ArticleDetailView, nav: "discover", crumbs: (slug) => {
